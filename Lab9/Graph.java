...
public class Graph<T> {
    ...
    void insertVertex(T newVertex) { // Exercise 1
        ...
        if (vertexCount == 0) {// create new graph
            firstList = temp;
            lastList = temp;
        } else {
    ...
    void deleteEdge(T fromData, T toData) { // Exercise 4
        ...
        if (fromAdjList.firstMember != null && toData.equals(fromAdjList.firstMember.adjVertex)) {
            // the first member of the list is the one to be deleted
    ...
    void deleteVertex(T vertex) { // Exercise 5
        ...
        while (currentAdjList != null) {
            if (!currentAdjList.data.equals(vertex)) { // check through every other vertex and remove edge
                deleteEdge(currentAdjList.data, vertex);
            } else {// found edge that have 'delete'
                if (currentAdjList == firstList) {// if the 'delete vertex' is on the first pos'
                    firstList = currentAdjList.nextList;
                } else if (currentAdjList == lastList) {// if the 'delete vertex' is at the last pos'
                    lastList = prevAdjList;
                    lastList.nextList = null;
                } else {// if the delete vertex is in the middle
